...
class ThermostatAccessory extends base_accessory_1.BaseAccessory {
    service;
    /** The pending coalesced write, if the dial is still moving. */
    pendingSetpointCelsius;
    coalesceTimer;
    ...
    readDisplayUnits() {
        ...
        const stored = this.accessory.context.scale;
        return stored === 'celsius'
            ? TemperatureDisplayUnits.CELSIUS
            : TemperatureDisplayUnits.FAHRENHEIT;
    }
    /** Honour the last observed scale before the first frame arrives. */
    applyStoredDisplayUnits() {
        const stored = this.accessory.context.scale;
        if (stored !== 'celsius' && stored !== 'fahrenheit') {
            return;
        }
        const { TemperatureDisplayUnits } = this.host.hap.Characteristic;
        this.publish(this.service, TemperatureDisplayUnits, stored === 'celsius'
            ? TemperatureDisplayUnits.CELSIUS
            : TemperatureDisplayUnits.FAHRENHEIT);
    }
    // --- Writes -----------------------------------------------------------------
    ...
    writeTargetTemperature(value) {
        ...
        }, settings_1.SETPOINT_COALESCE_MS);
    ...
    async flushSetpoint() {
        ...
        const native = (0, utils_1.resolveSetpoint)({
            celsius,
        ...
        try {
            ...
            this.optimisticTargetCelsius = (0, utils_1.nativeToCelsius)(native, observation.scale);
            this.logAction('SET', (0, utils_1.formatNative)(native, observation.scale));
        }
        catch (error) {
            this.optimisticTargetCelsius = undefined;
    ...
    restoreTargetState() {
        ...
        this.service.updateCharacteristic(TargetHeatingCoolingState, observation.power && this.readEnabled(observation)
            ? TargetHeatingCoolingState.HEAT
            : TargetHeatingCoolingState.OFF);
    }
    revertTargetTemperature() {
        const observation = this.host.observationFor(this.deviceId);
        const native = observation === undefined ? undefined : this.readSetpoint(observation);
        if (observation === undefined || native === undefined) {
            return;
        }
        this.service.updateCharacteristic(this.host.hap.Characteristic.TargetTemperature, (0, utils_1.nativeToCelsius)(native, observation.scale));
    }
    // --- Observation ------------------------------------------------------------
    updateFromObservation(observation, reason) {
        ...
        this.publish(this.service, Characteristic.CurrentHeatingCoolingState, heating
            ? Characteristic.CurrentHeatingCoolingState.HEAT
            : Characteristic.CurrentHeatingCoolingState.OFF);
        this.publish(this.service, Characteristic.TargetHeatingCoolingState, heating
            ? Characteristic.TargetHeatingCoolingState.HEAT
            : Characteristic.TargetHeatingCoolingState.OFF);
        const current = this.readTemperature(observation);
        if (current !== undefined) {
            this.publish(this.service, Characteristic.CurrentTemperature, (0, utils_1.nativeToCelsius)(current, observation.scale));
        }
        ...
        if (setpoint !== undefined && !this.isSettling(reason)) {
            ...
            // not it matches.
            this.optimisticTargetCelsius = undefined;
            this.publish(this.service, Characteristic.TargetTemperature, (0, utils_1.nativeToCelsius)(setpoint, observation.scale));
        }
        this.publish(this.service, Characteristic.TemperatureDisplayUnits, observation.scale === 'celsius'
     ...
     */
    publishLimits(observation) {
        const limits = this.readLimits(observation);
        if (limits === undefined || !(0, utils_1.isUsableRange)(limits.min, limits.max)) {
            return;
        }
        ...
        this.service.getCharacteristic(this.host.hap.Characteristic.TargetTemperature).setProps({
            ...
            minStep: utils_1.HOMEKIT_TEMPERATURE_STEP,
        });
        this.host.log.debug(`${(0, utils_1.forLog)(this.displayName)}: range ${(0, utils_1.formatNative)(limits.min, observation.scale)}`
            + ` to ${(0, utils_1.formatNative)(limits.max, observation.scale)}`);
    }
}
exports.ThermostatAccessory = ThermostatAccessory;
/** A native limit as the Celsius bound HAP wants, at display resolution. */
function toCelsiusBound(native, scale) {
    return (0, utils_1.round1)((0, utils_1.nativeToCelsius)(native, scale));
}
