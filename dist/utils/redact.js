...
exports.maskMacsIn = maskMacsIn;
exports.maskEmail = maskEmail;
exports.maskMac = maskMac;
...
function redactSecrets(value) {
    ...
    for (const pattern of PATTERNS) {
        ...
        result = result.replace(pattern, (_match, prefix) => (typeof prefix === 'string' ? `${prefix}${exports.REDACTED}` : exports.REDACTED));
    }
    return result;
}
/**
...
function redactObject(value) {
    ...
    for (const [key, child] of Object.entries(value)) {
        result[key] = SECRET_JSON_KEYS.includes(key) ? exports.REDACTED : redactObject(child);
    }
    return result;
}
/**
 ...
 */
function maskMacsIn(value, macAddresses) {
    ...
    const walk = (node) => {
        ...
        if (typeof node === 'string') {
            return substitutions.reduce((text, { pattern, replacement }) => text.replace(pattern, replacement), node);
        }
