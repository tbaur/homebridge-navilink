...
function validateAccount(config, errors, warnings) {
    ...
    if (rawEmail.length === 0 || rawPassword.length === 0) {
        errors.push('a NaviLink email address and password are required; open the plugin settings and sign in');
        return undefined;
    }
    ...
    if (password.length > settings_1.MAX_PASSWORD_LENGTH) {
        errors.push(`the configured NaviLink password is longer than ${settings_1.MAX_PASSWORD_LENGTH} characters`);
        return undefined;
    }
...
function resolveDeviceIdentity(input) {
    ...
    if (parsed === undefined) {
        input.problems.push(`${input.label} has no usable id (open the plugin settings and sign in again)`);
        return undefined;
    }
...
function validateConfig(config) {
    ...
    if (options.readOnly && devices.length > 0) {
        warnings.push('options.readOnly is on: every accessory will report state, and HomeKit will not be '
...
function resolveAccessories(devices, warnings) {
    ...
    for (const device of devices) {
        ...
        if (device.temperatureSensors) {
            accessories.push({ kind: 'dhwOutlet', deviceId: device.id, name: suffixName(device.name, 'Hot Water Out') }, { kind: 'dhwInlet', deviceId: device.id, name: suffixName(device.name, 'Hot Water In') }, { kind: 'heatSupply', deviceId: device.id, name: suffixName(device.name, 'Heating Flow') }, { kind: 'heatReturn', deviceId: device.id, name: suffixName(device.name, 'Heating Return') });
        }
