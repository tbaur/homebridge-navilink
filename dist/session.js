...
exports.NaviLinkSession = void 0;
...
class NaviLinkSession {
    ...
    observationFor(deviceId) {
        ...
        const maxAgeMs = this.options.statusIntervalSec * 1_000 * settings_1.STALE_OBSERVATION_INTERVALS;
     ...
     */
    applyOptimisticWrite(deviceId, patch) {
        ...
        const merged = { ...existing, ...patch, observedAt: existing.observedAt };
     ...
     */
    async publishControl(deviceId, spec) {
        const parsed = (0, identity_1.parseDeviceId)(deviceId);
        ...
        if (parsed === undefined || gateway === undefined || connection?.isConnected !== true) {
            throw new utils_1.ControlRejectedError('the appliance is not connected');
        }
        ...
        try {
            ...
            this.log.debug(`post-control status request failed: ${(0, utils_1.describeError)(error)}`);
        }
    }
    // --- Lifecycle --------------------------------------------------------------
    /**
    ...
    async runForever() {
        ...
        while (this.running) {
            ...
            const delay = (0, utils_1.backoffDelayMs)({
                attempt,
                baseMs: settings_1.RECONNECT_BACKOFF_BASE_MS,
    ...
    adoptGateways(listed, userSeq) {
        const wanted = new Set(this.options.devices
            .map((device) => (0, identity_1.parseDeviceId)(device.id)?.mac)
        ...
        for (const entry of listed) {
            ...
            this.gateways.set(entry.macAddress, {
                listed: entry,
                topics: (0, topics_1.buildTopics)(context),
                context,
     ...
     * to refuse to run.
     */
    async readFirmware(accessToken) {
        for (const gateway of this.gateways.values()) {
            try {
    ...
    buildConnection(tokens, signHostWithPort) {
        ...
        const options = {
            ...
            ...(first === undefined ? {} : {
                will: {
                    ...
                    payload: (0, protocol_1.appDisconnectWill)(first.context, first.topics.appConnection),
                    qos: 1,
                    retain: false,
                },
            }),
        };
        const connection = this.options.createConnection?.(options) ?? new mqtt_1.MqttConnection(options);
        connection.onMessage((message) => this.handleMessage(message.topic, message.payload));
        return connection;
    }
    async subscribeAll(connection) {
    ...
    handleConnectionClosed(from, error) {
    ...
    get hasStoppedPermanently() {
        return this.fatal !== undefined;
    }
    // --- Requests ---------------------------------------------------------------
    async requestChannelInfo(gateway) {
        const connection = this.connection;
        if (connection?.isConnected !== true) {
            return;
        }
        const frame = (0, protocol_1.channelInfoRequest)({
            context: gateway.context,
            topics: gateway.topics,
    ...
    async requestStatus(gateway, channelNumber) {
        const connection = this.connection;
        if (connection?.isConnected !== true) {
            return;
        }
        ...
        const frame = (0, protocol_1.channelStatusRequest)({
            context: gateway.context,
            topics: gateway.topics,
            responseTopic: (0, topics_1.responseTopic)(gateway.context, 'channelstatus'),
            channelNumber,
    ...
    handleChannelInfo(gateway, frame) {
        const channels = (0, channel_1.parseChannelInfo)(frame);
    ...
    handleChannelStatus(gateway, frame) {
        const status = (0, channel_1.parseChannelStatus)(frame);
        if (status === undefined) {
            return;
        }
        ...
        const reason = this.observations.has(deviceId)
    ...
    emit(deviceId, observation, reason) {
        for (const listener of this.observationListeners) {
            try {
                ...
                this.log.debug(`an observation listener threw: ${(0, utils_1.describeError)(error)}`);
            }
        }
    }
    notifyUnreachable(error) {
        ...
        for (const listener of this.unreachableListeners) {
            try {
                ...
                this.log.debug(`an unreachable listener threw: ${(0, utils_1.describeError)(listenerError)}`);
            }
        }
    }
    // --- Timers -----------------------------------------------------------------
        ...
        for (const deviceId of stale) {
            ...
            for (const listener of this.staleListeners) {
                try {
                    ...
                    this.log.debug(`a stale listener threw: ${(0, utils_1.describeError)(error)}`);
                }
            }
        }
    }
    ...
    warnMissingChannels(gateway) {
        for (const device of this.options.devices) {
            const parsed = (0, identity_1.parseDeviceId)(device.id);
