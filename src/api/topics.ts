...
function gatewayPrefix(identity: TopicIdentity): string {
  return `cmd/${identity.deviceType}/navilink-${identity.macAddress}/`
}

/** Build the session prefix. */
function sessionPrefix(identity: TopicIdentity): string {
...
export function responseTopic(identity: TopicIdentity, kind: FrameKind): string {
  return `${sessionPrefix(identity)}${kind}`
}

/** Build every topic one session needs for one gateway. */
export function buildTopics(identity: TopicIdentity): GatewayTopics {
 ...
 * reporting.
 */
export function classifyTopic(topic: string): FrameKind {
