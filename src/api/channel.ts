...
import { MAX_UNITS } from '../settings'
import type { ApplianceFamily, ChannelObservation, UnitReadings } from '../types'
import {
 ...
 */
const FAMILY_BY_UNIT_TYPE: Readonly<Record<number, ApplianceFamily>> = {
  1: 'NPE',
  2: 'NCB',
  3: 'NHB',
  4: 'CAS_NPE',
...
const DHW_FAMILIES: ReadonlySet<ApplianceFamily> = new Set<ApplianceFamily>([
  'NPE', 'NPN', 'NPE2', 'NCB', 'NFC', 'NCB_H', 'NFB', 'NVW',
  'CAS_NPE', 'CAS_NPN', 'CAS_NPE2', 'CAS_NFB', 'CAS_NVW',
])

/** Families that heat a space-heating loop. */
const SPACE_HEAT_FAMILIES: ReadonlySet<ApplianceFamily> = new Set<ApplianceFamily>([
 ...
 */
function isAbsentProbe(raw: unknown): boolean {
  return raw === 0
}
...
export function parseChannelInfo(frame: unknown): ChannelInfo[] {
  const list = asRecord(asRecord(asRecord(frame)?.response)?.channelInfo)?.channelList
  if (!Array.isArray(list)) {
    return []
  }
  const found = new Map<number, ChannelInfo>()
...
export function parseChannelStatus(frame: unknown): ChannelStatus | undefined {
...
export function scaleOf(info: ChannelInfo): TemperatureScale | undefined {
  return scaleFromTemperatureType(info.raw.temperatureType)
}

/** The family an appliance belongs to, from either frame. */
export function familyOf(raw: Readonly<Record<string, unknown>>): ApplianceFamily {
 ...
 */
export function formatFamily(family: ApplianceFamily): string {
 ...
 */
export function hasDomesticHotWater(family: ApplianceFamily): boolean {
  return DHW_FAMILIES.has(family)
}

/** True when this family heats a space-heating loop. */
export function hasSpaceHeating(family: ApplianceFamily): boolean {
 ...
 */
export function hasUsableHeatingLoop(
 ...
 */
export function decodeChannel(input: {
  info: ChannelInfo
  ...
  const temperature = (raw: unknown): number | undefined => (
    decodeWireTemperature(raw, scale)
  )
  const probe = (raw: unknown): number | undefined => (
    isAbsentProbe(raw) ? undefined : decodeWireTemperature(raw, scale)
  )
 ...
 * contact sensor.
 */
function firstUnitStatus(state: Readonly<Record<string, unknown>>): Record<string, unknown> | undefined {
  return unitStatusList(state)[0]
}

/** The first unit reporting a non-zero error, or the first unit if none is. */
function firstFaultedUnit(state: Readonly<Record<string, unknown>>): Record<string, unknown> | undefined {
  const units = unitStatusList(state)
  return units.find((unit) => (readInteger(unit.errorCode) ?? 0) !== 0) ?? units[0]
}

function readUnitReadings(
  unit: Record<string, unknown> | undefined,
  faulted: Record<string, unknown> | undefined = unit,
): UnitReadings {
 ...
 */
function clampUnitCount(value: number | undefined): number {
